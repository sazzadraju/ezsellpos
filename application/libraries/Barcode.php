...
class Barcode {
    ...
    function code128BarCode($code, $density = 1) {
        ...
        $width = (((11 * strlen($code)) + 35) * ($density / 80)); // density/72 determines bar width at image DPI of 72
        $height = ($width * .15 > .7) ? $width * .15 : .7;

        $px_width = round($width * 80);
        $px_height = ($height * 50);


        //$width			=	(100 * ($density)); // density/72 determines bar width at image DPI of 72
        //$height			=	35;
        //$px_width		=	$width;
        //$px_height		=	$height;
        ...
        for ($i = 0, $x = 0, $inc = round(($density / 80) * 100); $i < strlen($enc_str); $i++) {
