...
class Stock_Transfer_Model extends CI_Model {
    ...
    public function get_product_barcode($data=array()){
        $this->db->select("s.id_stock, p.product_name,p.product_code,p.id_product,sum(s.qty) qty,s.purchase_price,s.selling_price_act,s.expire_date,s.alert_date,s.rack_id,s.supplier_id,s.batch_no,sp.supplier_name,a.attribute_name");
        $this->db->from("stocks s");
        $this->db->join('products p', 'p.id_product = s.product_id', 'left');
        $this->db->join('suppliers sp', 'sp.id_supplier = s.supplier_id', 'left');
    ...
    function stock_transfer_details_list($stock_transfer_id) {
        ...
        $this->db->where('v.id_stock_mvt', $stock_transfer_id);
        $this->db->where('v.stock_mvt_type_id =', '7');
        $query = $this->db->get();
        $res = $query->result_array();
        return $res;
    }
    ...
    public function get_doc_file($stock_transfer_id){
        $this->db->select('id_document,file');
        $this->db->from('documents');
        $this->db->where('ref_id',$stock_transfer_id);
    ...
    function stock_transfer_list($params = array()) {
        $this->db->select('sm1.store_id as to_store,stock_mvts.store_id as from_store,users.fullname as  user_name,stock_mvts.id_stock_mvt,stock_mvts.notes, stock_mvts.dtt_stock_mvt, stock_mvts.invoice_no, documents.id_document, documents.file');
        ...
        $this->db->join('stock_mvts sm1', 'sm1.ref_id = stock_mvts.id_stock_mvt', 'LEFT');
        ...
        $this->db->where('stock_mvts.stock_mvt_type_id =', '7');
    ...
    public function stocks_details($stock_id) {
        $this->db->select("stocks.id_stock, stocks.product_id,p.product_code, stocks.supplier_id, stocks.rack_id, stocks.batch_no, stocks.qty, stocks.purchase_price, stocks.selling_price_est, stocks.selling_price_act, stocks.vat_rate, stocks.expire_date, stocks.store_id, stocks.alert_date, suppliers.supplier_name, racks.name");
        $this->db->from("stocks");
