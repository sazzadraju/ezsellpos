                                            ...
                                            <select class="select2" data-live-search="true" id="customer_id"
                                                ...
                                                foreach ($customers as $customer) {
                                                    if (empty($customer->_id_customer)) {
                                                        echo '<option csnm="' . $customer->full_name.'('.$customer->phone.')' . '" value="' . $customer->id_customer . '">' . $customer->full_name.'('.$customer->phone.')' . '</option>';
                                                    }
                                                }
                                                ?>
                                            </select>
...
<script>
    ...
    function searchFilter(page_num) {
        ...
        var customer_name=$('#customer_id option:selected').attr('csnm');
        var FromDate = $('#FromDate').val();
        var ToDate = $('#ToDate').val();
        if (check_valid() == 1) {
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url(); ?>customer-ledger-report/page-data',
                data: 'customer_id=' + customer_id + '&store_id=' + store_id + '&FromDate=' + FromDate + '&ToDate=' + ToDate+ '&customer_name=' + customer_name,
                beforeSend: function () {
                    $('.loading').show();
                },
    ...
    function print_view_show(page_num) {
        ...
        if (check_valid() == 1) {
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url(); ?>customer-ledger-report/print-page/',
                data: 'customer_id=' + customer_id + '&store_id=' + store_id + '&FromDate=' + FromDate + '&ToDate=' + ToDate+ '&customer_name=' + customer_name,
                beforeSend: function () {
                    $('.loading').show();
                },
    ...
    function csv_export() {
        ...
        if (check_valid() == 1) {
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url(); ?>reports/customer_ledger_report/create_csv_data',
                data: 'customer_id=' + customer_id + '&store_id=' + store_id + '&FromDate=' + FromDate + '&ToDate=' + ToDate+ '&customer_name=' + customer_name,
                beforeSend: function () {
                    $('.loading').show();
                },
