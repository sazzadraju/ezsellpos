                           ...
                           <div class="col-md-3 col-sm-12">
                            ...
                            <div class="col-md-3 col-sm-12">
                               <div class="form-group row">
                                  ...
                                  <div class="col-sm-12">
                                       <input type="checkbox" name="vat_type" value="1" class="form-control" id="vat_type">
                                       <label for="vat_type"><?= lang("with_non_vat_sale"); ?></label>
                                  </div>
                               </div>
                           </div>
...
<script>                                     
  function searchFilter(page_num) {
    ...
    var vat_type = ($('#vat_type:checked').val()==1)?1:0;
    $err_count = 1;
    $('#FromDate-error').html("");
    $('#ToDate-error').html("");
    if (FromDate == '') {
    ...
    if ($err_count == 1)  {
      $.ajax({
        type: 'POST',
        url: '<?php echo base_url(); ?>vat-report/page_data/' + page_num,
        data: 'page=' + page_num + '&store_id=' + store_id + '&FromDate=' + FromDate + '&ToDate=' + ToDate + '&vat_type=' + vat_type ,
        beforeSend: function () {
          $('.loading').show();
        },
  ...
  function searchFilter2(page_num) {
      page_num = page_num ? page_num : 0;
         ...
         if ($err_count == 1)  {
          $.ajax({
              type: 'POST',
              url: '<?php echo base_url(); ?>vat-report/print_page/' + page_num,
              data: 'page=' + page_num + '&store_id=' + store_id + '&FromDate=' + FromDate + '&ToDate=' + ToDate+ '&vat_type=' + vat_type ,
              beforeSend: function () {
                  $('.loading').show();
              },
