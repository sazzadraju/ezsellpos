        ...
        if (!empty($transactions['items'])):
            ...
            foreach ($transactions['items'] as $item):
                ...
                <tr>
                    ...
                    <td class="fit"><?php echo set_currency(comma_seperator($item['tot_amount'])); ?></td>
                    <td><?php echo get_key($qty_multipliers, $item['qty_multiplier']); ?></td>
                    <?php /*?><td><?php echo $item['type_name']; ?></td><?php */?>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="element-header margin-0"><?= lang("office_trx_details"); ?> <span class="close" data-dismiss="modal">&times;</span></h6>
            </div>
...
$(document).on('click', '.invoiceView', function () {
    ...
    var w = window.open('<?php echo base_url().'transactions/office_print_invoice';?>/'+oId,'name','width=800,height=500', '_blank');
    w.print({
        globalStyles: false,
        mediaPrint: true,
...
function viewDetaitls(id) {
    //console.log("<?php echo base_url() ?>account-management/office-trx-details/" + id,);
    $.ajax({
        url: "<?php echo base_url() ?>account-management/office-trx-details/" + id,
        type: "GET",
        beforeSend: function () {
            $('.loading').show();
        },
