                            ...
                            <td>
                                <select class="select2" data-live-search="true" id="supplier" name="supplier[<?= $id_p ?>][]" >
                                    ...
                                    foreach ($suppliers as $supplier) {
                                        $selected = ($id_supp == $supplier->id_supplier) ? 'selected' : '';
                                        echo '<option value="' . $supplier->id_supplier . '" ' . $selected . '>' . $supplier->supplier_name . '</option>';
                                    }
                                    ?>
                                </select>
                            </td>
                            <td><input style="width: 60px" type="text" onchange="change(<?= $count ?>)" name="qty[<?= $id_p ?>][]" class="form-control" onkeypress="return isNumber(event)" id="qty_<?= $count ?>"></td>
                            <td><input style="width: 100px" type="text" onchange="change(<?= $count ?>)" name="unit_price[<?= $id_p ?>][]" class="form-control" onkeypress="return isNumber(event)"  id="unit_price_<?= $count ?>" value="<?= $post['buy_price'] ?>"></td>
                            <td><input style="width: 100px" type="text" name="total_price[<?= $id_p ?>][]"  onchange="change_price(<?= $count ?>)" class="form-control" onkeypress="return isNumber(event)"  id="total_price_<?= $count ?>"></td>
