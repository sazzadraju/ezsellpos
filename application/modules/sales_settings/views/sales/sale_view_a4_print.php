            ...
            <div class="invoice">
               ...
               ?>
                ...
                <div class="in-from-to">
                    ...
                    <div class="to-inf">
                        <ul>
                            <li><span class="bold">ID:</span> <?= $customers[0]['customer_code'] ?></li>,
                            <li><span class="bold">Name:</span> <?= $customers[0]['full_name'] ?></li>,
                            <li><span class="bold">Phone:</span> <?= $customers[0]['phone'] ?></li>
                        </ul>
                    </div>
                </div>
                <?php 
                if(isset($replace_amount)){
                    ?>
                    <div style="width: 100%;">
                        <div style="width:40%;float:left;font-size:16px;font-weight:bold;padding:5px;">
                            Return Invoice: <?= $return_invoices[0]['invoice_no'] ?>
                        </div> 
                        <div style="width:40%;float:left;font-size:16px;font-weight:bold;padding:5px;">
                            Reference Invoice: <?= $return_invoices[0]['ref_sale_invoice'] ?>
                        </div>  
                    </div>
                    <table id="products" border="1">
                        ...
                        if (!empty($return_products)) {
                            foreach ($return_products as $row) {
                                ...
                                $free='<span style="background:green;color:white;padding:2px;border:1px solid;border-radius:8px;">Free</span>';
                                $free_data=($row['selling_price_act']==0)?$free:'';
                                ?>
                                <tr>
                                    <td><?php echo $free_data.' '.$code . '<span> ' . $row['product_name'] . '</span>'; ?></td>
                                    <?php
                                    echo '<td>';
                                    if($settings['brand']){
                    ...
                    <div class="table-break">
                        <div class="table-right">
                            <table id="products">
                                ...
                                if ($return_promotions) {
                                    foreach ($return_promotions as $promotion) {
                                        $type = $promotion->promotion_type_id;
                                        $dis=$promotion->discount_rate;
                                        $dis_amt=($product_amt*$dis)/100;
                                        echo '<tr><td style="text-align: right">'.$promo[$type].' (-)' . $promotion->discount_rate . '%</td>';
                                            echo '<td style="text-align: right">' . ($dis_amt) . '</td></tr>';
                                        $all_dis += $dis_amt;
                                    }
                                }
                                ?>
                                <tr>
                                    <td style="text-align: right"><span class="bold">Return Total</span></td>
                                    <td style="text-align: right"><span class="bold"><?php echo number_format(($product_amt - $all_dis),2); ?></span></td>
                                </tr> 
                            </table>
                        </div>
                    </div>
                        ...
                        <table id="products">
                            ...
                            <?php
                                ...
                                if (isset($cash_pay)) {
                                        ...
                                        if (isset($replace_amount)) {
                                            echo '<tr><td style="text-align: right">Replace Amount</td>';
                                            echo '<td style="text-align: right">' . floatVal($replace_amount) . '</td></tr>';
                                            $all_paid_amt += $replace_amount;
                                        }
                                ...
                                <?php
                                $due_amt=($dalivery_total+$g_total)-$g_total_paid;
                                ?>
                                <tr>
